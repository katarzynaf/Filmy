...
stats_to_one_row <- function(link, user_stats = user_rating_stats(link), votes_stats = rating_stats(link)) {
      ...
      table_columns <- c("Id", "Title", "Overall_Rating", "Votes", "Males", "Females", "Aged_under_18", "Males_under_18", "Females_under_18", "Aged_18-29",
      ...
      id <- stri_extract_all_regex(link,"(?<=/title/).+",simplify=TRUE)
      table_row <- c(id, title(link), rep("-1", length(table_columns) - 2))
