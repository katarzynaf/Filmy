#' Statystyki jednego filmu
#'
#' Funkcja ulatwia pobranie i zapis wszystkich statystyk o danym filmie do jednego pliku.
#'
#' @aliases stats_to_one_row
#' @param link link do strony glownej filmu na IMDb'ie, \code{typu character}
#' @return jeden wiersz formatu ramki danych ze statystykami dotyczacymi filmu.
#' @import rvest
#' @import stringi
#' @export


stats_to_one_row <- function(link) {
      # wyciagniecie statystyk funkcjami z pakietu
      user_stats <- user_rating_stats(link)
      votes_stats <- rating_stats(link)
      ...
      table_row <- c(movie_title(link), rep("-1", length(table_columns) - 1))
