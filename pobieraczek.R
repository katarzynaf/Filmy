# widze to jakos tak, ze jak sie dzielimy rokami, to wrzucamy sobie csv-ki z naszymi latami do jednego folderu i on z niego wszystko
# automatycznie laczy i pobiera :) poprawilam na te potrzeby fnkcje merge.csv() (tzn. zdazyl to zrobic Piotrek lub Martyna ale jeszcze dopisalam
# Wam tam '/' :P)
library(XML)
library(rvest)
library(dplyr)
...
pobieraczek <- function(file_destination, folder_z_moja_czescia) {
    ...
    while (i <= liczba_linkow) {
        ...
        d <- keywords(link)
        suppressWarnings(write.table(as.data.frame(c(a, b, c, d)), file = dest_file, append = TRUE, sep = ";", row.names = FALSE, col.names = !(file.exists(dest_file))))
        i = i + 1
    }
    close(con)
    cat("\nDone\n")
